# Think Bot 功能、UI 与交互描述文档

本文档旨在全面描述 Think Bot 浏览器扩展的核心功能、用户界面 (UI) 和交互流程，为项目未来的重构或重新开发提供清晰、准确的蓝图。
...
1.  顶部控制和提取内容展示:
    顶部控制栏
    -   左侧:
        -   内容提取方式切换: 提供 "Jina AI" 和 "Readability.js" (R) 两种模式的切换按钮。
...
2.  左区导航栏，点击切换右区的配置项
    ...
    -   快捷指令 (Quick Input Tabs):
        ...
        -   每项包含“按钮显示文本”、“实际发送的指令内容”和一个“自动触发”的开关。
        -   提供添加和删除操作。
...
-   Sortable.min.js:
    -   用途: 实现拖拽排序功能。
    -   位置: 仅在选项页面 (Options) 中使用，用于让用户可以方便地通过拖拽来调整“语言模型”和“快捷指令”列表的顺序。
